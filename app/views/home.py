...
class HomePage():
    ...
    def render(self):
        ...
        homeLabel = Label(self.window, text=f"Backup Management", font=('Poppins', 18, 'bold'), bg="white")
        homeLabel.pack(padx=10, pady=10)
        
        custom_font=('Poppins', 10)
        intro_text = "Welcome to upGIT, the best backup management tool for your GitHub repositories."
        homeLabel = Message(self.window, text=intro_text, font=('Poppins', 11), bg="white", width=400, anchor="center", justify="center")
        homeLabel.pack(padx=10, pady=0)
        
        self.folder_path = StringVar()
        Label(self.window, text="Select Folder to Backup", bg="white", font=custom_font).pack(anchor=W, padx=10, pady=5)
        folder_frame = Frame(self.window, bg="white")
        folder_frame.pack(fill=X, padx=10)
        Entry(folder_frame, textvariable=self.folder_path, width=50, highlightthickness=2, highlightcolor="gray70", bg="white", font=custom_font).pack(side=LEFT, padx=5)
        ...
        Label(self.window, text="Set Backup Frequency", bg="white", font=custom_font).pack(anchor=W, padx=10, pady=5)
        self.frequencyEntry = Entry(self.window, width=15, highlightthickness=2, highlightcolor="gray70", bg="white")
        self.frequencyEntry.place(relx=0.028, rely=0.5)
        ...
        backupOptions.place(relx=0.43, rely=0.49)

        # Branch Selection Section
        self.branch_var = StringVar(value="None")
        Label(self.window, text="Choose a Branch", bg="white", font=custom_font).place(relx=0.028, rely=0.6)
        ...
        option_menu.place(relx=0.028, rely=0.67)
        Button(self.window, font=custom_font, text="New", bg="RoyalBlue1", bd=0, fg="white", command=self.open_create_branch, width=10, anchor=CENTER, cursor="hand2").place(relx=0.47, rely=0.67)

        # Action Buttons
        Button(self.window, text="Start Backup", font=custom_font, command=self.start_backup, bg="#4CAF50", fg="white", bd=0, width=16, cursor="hand2").place(relx=0.028, rely=0.8)
        Button(self.window, text="Cancel", font=custom_font, command=self.window.quit, bg="#f44336", fg="white", bd=0, width=16, cursor="hand2").place(relx=0.7, rely=0.8)
